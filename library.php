              ...
              <div class="box">
                ...
                <div class="box-body">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      <?php 
                        $get_books="SELECT * FROM Library_bookbank  order by 	PRICE ASC";
                        ...
                        do
                       {
                         $SERIALNO=$get_books_fetch['SERIALNO'];
                         $AUTHORNAME=$get_books_fetch['AUTHORNAME'];
                         $BOOKNAME=$get_books_fetch['BOOKNAME'];
                         ...
                         echo '<tr>
                                ...
                                <td>'.$BOOKNAME.'</td>
                                <td>'.$PUBLISHER.'</td>';
                                $sql="SELECT * from book_issue where ISBN='$SERIALNO' ORDER BY id DESC";
                                $query=mysqli_query($conn,$sql);
                                $num=mysqli_num_rows($query);
                                if ($num==0) {
                                	 echo  '<td><a href="qrcode_lib.php?SERIALNO='.$SERIALNO.'" ><span class="label label-success pull-middle">Available</span></a></td>';
                                }
                                else
                                {
                          ...
                          {
                             echo  '<td><span class="label label-warning pull-middle">ISSUED</span>&nbsp<a href="qrcode_lib.php?SERIALNO='.$SERIALNO.'&p=r" ><span class="label label-success pull-middle">Return</span></a></td>';
...
<?php 
if (isset($_GET['userid']) && isset($_GET['SERIALNO'])  ) 
{
	...
	if (isset($_GET['p'])) {
		...
		$check="SELECT * from book_issue where SERIALNO='$SERIALNO' and username='$user' order by id DESC";
		...
		else
		{
			...
			$purpose="Returned";
			$sql="INSERT INTO book_issue (id, date, time, ISBN, username, Purpose) VALUES (NULL, '$date', '$time', '$SERIALNO', '$user', '$purpose')";
			$query=mysqli_query($conn,$sql);
			if ($query) {
...
$query=mysqli_query($conn,$sql);
if ($query) {
	echo '<script>alert("Book Issued"); window.location="Library.php"</script>';
}
